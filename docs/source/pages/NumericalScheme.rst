...
The figure below shows the full operator call graph generated by ``--debug-graph`` for the Poiseuille example. Operators marked with a red dot (•) modify the ``fields`` slot (LBM distributions). The colour of each block indicates the execution phase: **green** = initialize, **orange** = loop, **grey** = finalize.

.. image:: ../_static/HippoLBMGraph.png
   :align: center
   :width: 70%

The graph makes the operator tree explicit:

- During **initialize**, the grid is allocated (``define_grid_3dq19``), obstacles are registered (``setup_obstacles`` → ``register_quadrics ×3``), distributions are set (``set_distribution``), and a first ParaView output is written.
- During the **loop**, each iteration runs ``macro_variables`` → ``bgk`` → ``wall_bounce_back`` → ``streaming`` → ``neumann ×2`` (via ``boundary_conditions``), followed by conditional I/O (``dump_paraview_if_triggered``, ``print_log_if_triggered``) controlled by ``nth_timestep`` triggers.
- After the loop, a final ``write_paraview``, ``checker``, and ``finalize_cuda`` conclude the simulation.
